...
class TraceRecord:
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "run_id": self.run_id, "trace_id": self.trace_id, "role": self.role,
            "created_at": self.created_at, "content_entry": self.content_entry,
            "derived_from": list(self.derived_from), "set_ids": list(self.set_ids),
            ...
            "status": self.status, "runtime_config": self.runtime_config,
            "created_by": self.created_by, "tags": list(self.tags),
        }

    @classmethod
    def from_mapping(cls, data: dict[str, Any]) -> "TraceRecord":
        return cls(
            run_id=str(data["run_id"]), trace_id=str(data["trace_id"]),
